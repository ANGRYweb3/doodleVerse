...
interface HashConnectComponentProps {
  ...
  onDisconnect: () => void;
  accountId: string;
  onConnectWalletRef?: (connectFn: () => void) => void;
}
...
const HashConnectComponent: React.FC<HashConnectComponentProps> = ({ 
  ...
  const [, setPairingData] = useState<SessionData | null>(null);
  ...
  }, [hashConnect, onConnect, onDisconnect, retryCount]);

  const connectWallet = useCallback(() => {
    const debouncedConnect = createDebounce(async () => {
    ...
    debouncedConnect();
  }, [hashConnect, isInitialized, isConnecting]);
  ...
  // Pass connectWallet function to parent component
  useEffect(() => {
    if (onConnectWalletRef && connectWallet) {
      onConnectWalletRef(connectWallet);
    }
  }, [onConnectWalletRef, connectWallet]);
